...
def test_udf_u_mul_e(G):
    return time_udf(G, udf_u_mul_e)

def test_udf_u_add_e(G):
    return time_udf(G, udf_u_add_e)

def time_builtin(G, builtin):
    dur = []
    for i in range(50):
        if i >= 5:
            t0 = time.time()
        G.update_all(builtin, dgl.function.sum('m', 'hh'))
        if i >= 5:
            dur.append(time.time() - t0)
    return np.average(dur)

def test_builtin_u_mul_e(G):
    return time_builtin(G, dgl.function.u_mul_e('h', 'w', 'm'))

def test_builtin_u_add_e(G):
    return time_builtin(G, dgl.function.u_add_e('h', 'w', 'm'))
'''
...
'''
def test(time_udf, time_builtin):
    G = dgl.DGLGraph(adj)
...
test(test_udf_u_mul_e, test_builtin_u_mul_e)
test(test_udf_u_add_e, test_builtin_u_add_e)
